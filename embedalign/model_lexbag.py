...
from dgm4nlp.tf.encoder import EmbeddingEncoder
from dgm4nlp.tf.encoder import PrefixEncoder
import logging
...
class EmbedAlignModelLexBag:
    ...
    def _inference(self):
        ...
        ################################
        # Inference component q(S|x_1^m)
        h_for_s = self._qs_encoder(x_embedded)
        ...
        # Inference components q(Z|x_1^m, s)
        h_for_z = self._qz_encoder(x_embedded)
    ...
    def _build_model(self):
        ...
        x_len = tf.reduce_sum(tf.sign(self.x), axis=1)
        # [B]
        y_len = tf.reduce_sum(tf.sign(self.y), axis=1)

        # [B, ds], [B, M, dz]
        s, z = self._inference()

        #########################
        # Generative components P(Z|z_prev)
        z_prev = tf.pad(z, [[0, 0], [1, 0], [0, 0]])[:, :-1, :]  # append z0 to the left and remove z_m from the right
        # Predict parameters
        # [B, M, dz]
        gen_z_mean = FeedForwardEncoder(
            num_units=self.dz,
            hidden_layers=[],
            name='Zprev-Z-Mean'
        )(z_prev)
        # [B, M, dz]
        gen_z_log_var = FeedForwardEncoder(
            num_units=self.dz,
            hidden_layers=[],
            name='Zprev-Z-LogVar'
        )(z_prev)
        ...
        #########################
        # Generative components P(X|z)
        logging.info('X conditions on z: P(X_i|z_i)')
        ...
        # [B, M, Vx]
        px_z = tf.reshape(tf.nn.softmax(logits_x), [batch_size, longest_x, -1], name='px_z')

        ##########################
        # Generative component P(Y|s)
        logging.info('Y conditions on s: P(Y_j|s)')
        # Y decoder (given s)
        with tf.variable_scope('logit-y'):
            # [B * 1, Vy|S], [B * N]
            logits_y, targets_y = logit_layer_for_bitext(
                nb_classes=self.vy,
                inputs=tf.reshape(s, [batch_size, 1, self.ds]),  # [B, 1, ds]
                outputs=self.y,  # [B, N]
                dim=self.ds,
        ...
        # [B, Vy|S]
        py_s = tf.nn.softmax(logits_y, name='py_s')

        ##########################
        # Generative components P(A|y<, z)
        if self.alignment_component == 'uniform':
            # [B, M]
            pa_z = tf.div(x_mask, tf.cast(tf.expand_dims(x_len, -1), tf.float32))
            # [B, 1, M]
            pa_z = tf.expand_dims(pa_z, 1)
            # [B, N, M]
            pa_z = tf.tile(pa_z, [1, longest_y, 1])
        elif self.alignment_component == 'attention':
            # Embed words in y
            # [B, N, dy=dx]
            y_embedded = EmbeddingEncoder(
                num_units=self.dy,
                vocab_size=self.vy,
                name='Ey'
            )(self.y)
            # [B, N, dh]
            y_history = PrefixEncoder(
                num_units=self.dy,
                cell_type='lstm',
                name='prefix_y')(inputs=y_embedded, lengths=y_len)

            # Prediction of Categorical parameters of P(A_j|z_1^m, y_<j) via attention
            # [B, N, M]
            logits = tf.matmul(
                tf.layers.dense(y_history, self.dz),  # [B, N, dz]
                tf.layers.dense(z, self.dz),             # [B, M, dz]
                transpose_b=True
            )
            # mask invalid logits
            logits = tf.where(
                # make the boolean mask [B, N, M]
                condition=tf.tile(
                ...
                x=logits,
                y=tf.ones(shape=[batch_size, longest_y, longest_x]) * float('-inf')
            )
            # Normalise categorical parameters
            # [B, N, M]
            pa_z = tf.nn.softmax(logits)
        ...
        # [B, M]
        self.pa_x = tf.identity(pa_z, name='pa_x')
        # [B, M, Vx]
